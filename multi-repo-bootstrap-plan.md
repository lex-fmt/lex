...
2. **Standard checkout location** — `/tmp/lex-fmt/<repo>`. Documented in user-level `CLAUDE.md` so agents grep here before re-cloning.
...
- Concurrent runs: file-lock under `/tmp/lex-fmt/.lock` per-repo so two parallel agent invocations don't trip over each other. Probably overkill for the actual use case but cheap.
...
> (2) Task is a planning / proposal / cross-cascade analysis spanning ≥2 lex-fmt repos.
...
5. When done, do not clean up `/tmp/lex-fmt/`. Subsequent sessions / steps benefit from the cache.
...
1. **Location root**: `/tmp/lex-fmt/` (proposed) vs `~/lex-fmt/` vs `/workspace/lex-fmt/`? `/tmp` matches existing user-level conventions and survives within a session, which is all we need.
