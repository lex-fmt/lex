...
pub(super) fn emit_pipe_table(serializer: &mut LexSerializer, table: &Table) {
    ...
    if multiline {
        emit_multiline_table(serializer, &grid, &widths, &aligns, col_count, header_count);
    } else {
        for (idx, slots) in grid.iter().enumerate() {
            for line in format_grid_row_lines(slots, &widths, col_count) {
...
fn format_grid_row_lines(slots: &[Slot], widths: &[usize], col_count: usize) -> Vec<String> {
    // Each column's physical lines (empty padding for columns a short row omits).
    let columns: Vec<Vec<&str>> = (0..col_count)
        .map(|col| slots.get(col).map(Slot::lines).unwrap_or_else(|| vec![""]))
        .collect();
    ...
    (0..height)
        .map(|line_idx| {
            ...
            for (col, &width) in widths.iter().enumerate().take(col_count) {
                let text = columns[col].get(line_idx).copied().unwrap_or("");
