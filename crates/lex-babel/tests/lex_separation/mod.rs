...
/// Regression guard (lex#782): `lex → lex` output is **byte-identical to
/// pre-change** across a representative corpus of real Lex documents.
///
/// The golden files under `tests/fixtures/lex_golden/` were captured by running
/// the serializer on the epic base commit (before this slice) over the comms
/// corpus (trifecta + benchmark + element `.lex` fixtures) — one `.golden` per
/// file that formats cleanly, named `<corpus>__<stem>.golden`. This test formats
/// each corpus file with the *current* serializer and asserts the output equals
/// its golden. Because a lex-sourced AST already carries a `BlankLineGroup` ≥ the
/// structural minimum between blocks, the max-composing matrix must not perturb
/// any of these — a single altered blank anywhere in the corpus fails here.
///
/// (Idempotence was rejected as the mechanism: a few corpus files, e.g.
/// `080-gentle-introduction.lex`, are already non-idempotent on the base commit
/// for unrelated formatter reasons, so idempotence cannot isolate *this* change.)
///
/// Exactly one golden was intentionally regenerated for this slice:
/// `elements__definition.golden`. Removing the lex#505 verbatim band-aid drops a
/// spurious blank it used to insert between two *adjacent verbatim blocks*
/// (Verbatim→Verbatim = 0 in the matrix; the source has no blank there). The new
/// output stays faithful (`format_invariants` is green) and is now idempotent
/// where the base output was not. Every other corpus file is byte-identical to
/// pre-change.
#[test]
fn comms_corpus_lex_to_lex_matches_pre_change_golden() {
    let roots = [
        "../../comms/specs/trifecta",
        "../../comms/specs/benchmark",
        "../../comms/specs/elements",
    ];
    let golden_dir = std::path::Path::new("tests/fixtures/lex_golden");
    let mut checked = 0usize;
    for root in roots {
        let dir = std::path::Path::new(root);
        let entries = match std::fs::read_dir(dir) {
            Ok(e) => e,
            Err(_) => continue, // corpus not present in this checkout
        };
        for entry in entries.flatten() {
            ...
            let root_tag = dir.file_name().unwrap().to_str().unwrap();
            let golden_path = golden_dir.join(format!("{root_tag}__{stem}.golden"));
            let Ok(expected) = std::fs::read_to_string(&golden_path) else {
                // No golden means the base serializer could not format this file
                // (malformed fixture); nothing to guard against.
                continue;
            };
            let src = std::fs::read_to_string(&path).expect("read corpus file");
            let got = format_lex_source(&src)
                .unwrap_or_else(|e| panic!("[{}] format failed: {e}", path.display()));
            assert_eq!(
                got,
                expected,
                "lex -> lex output changed vs pre-change golden for {}",
                path.display()
            );
            checked += 1;
        }
    ...
    assert!(
        checked > 0,
        "expected at least one corpus .lex golden to be checked"
    );
}
